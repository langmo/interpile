...
uimenu(editMenu, 'Label',...
    'Invariant Factors', ...
    'Callback', @(figH, ~)invariantFactors(figH), 'Separator','off');
...
uimenu(transformMenu, 'Label',...
        'To recurrent', ...
        'Callback', @(figH, ~)plotMain(toRecurrent(getPile(figH)), figH));
uimenu(transformMenu, 'Label',...
        'To non-recurrent', ...
...
uicontrol('Style','Radio','String','Wave',...
    ...
    'Tag', 'modeWave',...
    'BackgroundColor', [1,1,1]);
uicontrol('Style','Radio','String','Toggle mask',...
...
uicontrol('Style','checkbox','String','Auto-topple',...
    'Units', 'centimeters','pos',[0.25,1.5,3,0.5],'parent', modeH,'HandleVisibility','off',...
    'Tag', 'modeAutoTopple',...
...
function countRecurrent(figH)
    ...
    h = msgbox({...
                ...
                sprintf('Calculation Time: %.2fs', timePotential) ....
              }, 'Number of Recurrent States');
    setWindowIcon(h);
    h.Color = ones(1,3);
end
function invariantFactors(figH)
    S = getPile(figH);
    if max(size(S)) > 10
        choice = questdlg('The calculation of the invariant factors of the sandpile group for domains with a width or height larger than 10 can take very long. Continue anyways?', ...
            'Long calculations', ...
            'Yes','No', 'No');
        if strcmpi(choice, 'No')
            return;
    ...
    calcTime = tic();
    factors = calculateInvariantFactors(mask, 'method', 'laplacian');
    factors(factors==1)=[];
    time = toc(calcTime);
    for f = 1:length(factors)
        if f==1
            factorsStringPotential = sprintf('%1.0f', factors(f));
        else
            factorsStringPotential = sprintf('%s, %1.0f', factorsStringPotential, factors(f));
        end
    ...
    h = msgbox({...
                ...
                sprintf('invariant factors: %s', factorsStringPotential), ...
                sprintf('Calculation Time: %.2fs', time) ....
              }, 'Invariant Factors');
    setWindowIcon(h);
    h.Color = ones(1,3);
end
...
function onResize(figH, ~)
    ...
    modeGroup.Position = [figureDim(1)-optionsWidth, figureDim(2)-6, optionsWidth-0.25, 5.75];
end
...
function dropParticle(figH, ~)
    ...
    if xData > 0 && xData <= width && yData > 0 && yData <= height
        ...
        elseif selectedChoice == findall(figH, 'Tag', 'modeMask')
            if isinf(S(yData, xData))
                S(yData, xData) = 0;
            else
                S(yData, xData) = -inf;
