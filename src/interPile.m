...
function countRecurrent(figH)
    ...
    tic();
    numStatesLaplacian = numRecurrentStates(mask, 'laplacian');
    timeLaplacian = toc();
    tic();
    numStatesPotential = numRecurrentStates(mask, 'potential');
    timePotential = toc();
    
    h = msgbox({...
                'via graph laplacian', '--------------------------------------', sprintf('#recurrent: %d', numStatesLaplacian), ...
                sprintf('#recurrent/#total: %.5f%%', numStatesLaplacian/numStatesTotal*100), ....
                sprintf('#recurrent^(1/|Gamma|): %.5f', nthroot( numStatesLaplacian, numVertices)), ....
                sprintf('Caclulation Time: %.2fs', timeLaplacian), ....
                '',...
                'via potentials',      '--------------------------------------', sprintf('#recurrent: %d', numStatesPotential), ...
